...
internal interface ITableInternal
{
    ...
    void WriteSnapshot(BufferWriter writer, BufferWriter scratch);

    void LoadSnapshot(ref BufferReader reader);
...
/// Mutations flow through the owning <see cref="Database"/>'s transaction machinery.
/// </summary>
public sealed class Table<TKey, TRow> : ITableInternal
    where TKey : notnull
{
    ...
    private readonly IIndexStore<TRow>[] _indexStores;

    internal Table(Database db, TableDescriptor<TKey, TRow> descriptor)
    {
        _db = db;
        Descriptor = descriptor;
        _indexStores = new IIndexStore<TRow>[descriptor.Indexes.Count];
    ...
    public TableDescriptor<TKey, TRow> Descriptor { get; }
    ...
    public IEnumerable<TRow> Lookup<TIndexKey>(HashIndexDefinition<TRow, TIndexKey> index, TIndexKey key) =>
        ((HashIndexStore<TRow, TIndexKey>)GetIndexStore(index)).Lookup(key);

    /// <summary>Number of rows whose indexed value equals <paramref name="key"/>.</summary>
    public int CountBy<TIndexKey>(HashIndexDefinition<TRow, TIndexKey> index, TIndexKey key) =>
        ((HashIndexStore<TRow, TIndexKey>)GetIndexStore(index)).CountOf(key);

    /// <summary>All rows sorted by the ordered index.</summary>
    public IEnumerable<TRow> OrderBy<TIndexKey>(OrderedIndexDefinition<TRow, TIndexKey> index, bool descending = false) =>
        ((OrderedIndexStore<TRow, TIndexKey>)GetIndexStore(index)).All(descending);
    ...
    public IEnumerable<TRow> Range<TIndexKey>(
        ...
        ((OrderedIndexStore<TRow, TIndexKey>)GetIndexStore(index))
    ...
    /// <summary>Adds a new row; throws <see cref="DuplicateKeyException"/> if the key exists.</summary>
    ...
    internal IIndexStore<TRow> GetIndexStore(IndexDefinition<TRow> definition)
    {
        foreach (var store in _indexStores)
        {
    ...
    // ---- ITableInternal (replay / snapshot) ----

    void ITableInternal.ReplaySet(ReadOnlySpan<byte> keyBytes, ReadOnlySpan<byte> rowBytes)
    {
        var keyReader = new BufferReader(keyBytes);
        var key = Descriptor.KeyReader(ref keyReader);
        var rowReader = new BufferReader(rowBytes);
        var row = Descriptor.RowReader(ref rowReader);
        SetEntry(key, row, _db.NextSeq());
    }

    void ITableInternal.ReplayDelete(ReadOnlySpan<byte> keyBytes)
    {
        var keyReader = new BufferReader(keyBytes);
        var key = Descriptor.KeyReader(ref keyReader);
        RemoveEntry(key);
    ...
    void ITableInternal.WriteSnapshot(BufferWriter writer, BufferWriter scratch)
    {
    ...
    void ITableInternal.LoadSnapshot(ref BufferReader reader)
    {
    ...
    private sealed class SetOp : ITxnOp
    {
        ...
        public SetOp(Table<TKey, TRow> table, TKey key, TRow row, bool hadOld, TRow? oldRow, long oldSeq)
        {
            _table = table;
            _key = key;
            _row = row;
        ...
        public void EncodeRedo(BufferWriter payload, BufferWriter scratch)
        {
            payload.WriteByte(WalOp.Set);
    ...
    private sealed class DeleteOp : ITxnOp
    {
        private readonly Table<TKey, TRow> _table;
        private readonly TKey _key;
        private readonly TRow _oldRow;
        ...
        public DeleteOp(Table<TKey, TRow> table, TKey key, TRow oldRow, long oldSeq)
        {
            _table = table;
            _key = key;
            _oldRow = oldRow;
        ...
        public void EncodeRedo(BufferWriter payload, BufferWriter scratch)
        {
            payload.WriteByte(WalOp.Delete);
            payload.WriteString(_table.Name);
            scratch.Reset();
            _table.Descriptor.KeyWriter(scratch, _key);
            payload.WriteBytes(scratch.WrittenSpan);
